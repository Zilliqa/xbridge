...
contract deployZilbridgeTokenManagers is Script, MainnetConfig, TokenManagerDeployerUtil {
  function run() external {
    ...
    LockAndReleaseOrNativeTokenManagerUpgradeableV3 lockAndReleaseOrNativeTokenManager = deployLockAndReleaseOrNativeTokenManager(zilChainGatewayAddress, fees);
    LockProxyTokenManagerUpgradeableV3 lockProxyTokenManager = deployLockProxyTokenManager(zilChainGatewayAddress, fees);
    console.log(" zilLockAndReleaseOrNativeTokenManagerUpgradeable = %s;", address(lockAndReleaseOrNativeTokenManager));
    console.log(" zilLockProxyTokenManager = %s;", address(lockProxyTokenManager));
    vm.stopBroadcast();
  }
}
