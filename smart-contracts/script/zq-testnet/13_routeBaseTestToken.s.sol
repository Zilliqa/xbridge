...
contract Deployment is Script, TestnetConfig {
  function run() external {
    ...
    LockAndReleaseOrNativeTokenManagerUpgradeableV5 zilliqaTokenManager = LockAndReleaseOrNativeTokenManagerUpgradeableV5(payable(zqLockAndReleaseOrNativeTokenManagerAddress));
    ...
    ITokenManagerStructs.RemoteToken memory router = ITokenManagerStructs.RemoteToken({
     token: address(basezbTestTokenAddress),
     tokenManager: address(baseMintAndBurnTokenManagerUpgradeable),
     chainId: baseChainId});
    zilliqaTokenManager.registerToken(address(zqZRC2EVMAddress), router);
  }
}
