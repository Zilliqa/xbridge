...
contract Deployment is Script, MainnetConfig {
    function run() external {
        ...
        address vmImplementation = address(
            new ValidatorManagerUpgradeable{salt: "eth"}()
        );
        ...
        address vmProxy = address(
            new ERC1967Proxy{salt: "eth"}(vmImplementation, vmInitCall)
        );
        ...
        address cgImplementation = address(
            new ChainGatewayUpgradeable{salt: "eth"}()
        );
        ...
        address cgProxy = address(
            new ERC1967Proxy{salt: "eth"}(cgImplementation, cgInitCall)
        );
        ...
        console.log("address public constant ethValidatorManager = %s",address(validatorManager));
        console.log(" address public constant ethChainGatewayAddress = %s", address(chainGateway));
        vm.stopBroadcast();
    }
}
