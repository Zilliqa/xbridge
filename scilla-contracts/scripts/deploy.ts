...
async function main() {
  ...
  console.log(`    address public constant zqBridgedERC20Address = address(${addressToConstant(tokenContract.address)})`);
  ...
  console.log(`    address public constant zqBridgedBNBAddress = address(${addressToConstant(nativeBridgedContract.address)})`);

  let local = await hre.deployScillaContract("FungibleToken", account.address, "zq_native Test", "ZTST", 3, 1_000_000_000);
  console.log(`    address public constant zqZRC2Address = address(${addressToConstant(local.address)})`);
